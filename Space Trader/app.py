...
def settings():
    ...
    if settingform.validate_on_submit():
        ...
        return redirect(
            url_for(
                "skillpoints",
                name=settingform.name.data,
                diff=settingform.diff.data,
                sp=sp,
                credits=credits,
            )
        )
...
@app.route(
    "/skillpoints?name=<name>&diff=<diff>&sp=<sp>&credits=<credits>",
    methods=["GET", "POST"],
)
...
@app.route(
    "/confirm?name=<name>&diff=<diff>&sp=<sp>&sp1=<sp1>&sp2=<sp2>&sp3=<sp3>&sp4=<sp4>&credits=<credits>",
    methods=["GET"],
)
def confirm(name, diff, sp, sp1, sp2, sp3, sp4, credits):
    ...
    if confirmform.is_submitted:
        ...
        player = Player(
            name, pSkillPoints, credits, testUniverse.regionList[randRegion]
        )
...
def start(startingRegion):
    ...
    return render_template(
        "start.html", universe=testUniverse, randRegionIndex=int(startingRegion)
    )
